package de.thkoeln.fherborn.fearlesschange.db.migration
...
class CardInitializer(val context: Context): DataInitializer {
    override fun onInsert() {
        CardDatabase.getInstance(context).cardDao().insertCards(INITIAL_CARDS)
    }
}
...
private val INITIAL_CARDKEYWORDS = listOf(
        CardKeyword(0, "Keyword 1"),
        CardKeyword(0, "Keyword 2"),
        CardKeyword(1, "Keyword 2")
)

private val INITIAL_KEYWORDS = listOf(
        Keyword("Keyword 1"),
        Keyword("Keyword 2"),
        Keyword("Keyword 3")
)