 ...
 */
class DefaultCardPreviewBehavior(val activity: AppCompatActivity?, private val popupName: String = "CardPopup") : DefaultCardStatisticBehavior(activity) {
        ...
        card?.let {
            if (action == CardViewAction.CARD_CLICKED) {
                activity?.supportFragmentManager?.let { fm ->
                    ...
                    val cardPopup = CardPopup.newInstance(cardId = it.id)
